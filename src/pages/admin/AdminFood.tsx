...
const CATS = ['Breakfast','Starters','Main Courses','Kenyan Cuisine','Seafood','Chicken','Beef','Vegetarian','Snacks','Desserts']
...
export default function AdminFood() {
  ...
  const load = () => { setLoading(true); fetch('/api/food').then(r => r.json()).then(d => { setItems(Array.isArray(d) ? d : []); setLoading(false) }) }
  useEffect(load, [])

  const openNew = () => setEditing({ id: '', name: '', description: '', price: 500, image_url: '', category: CATS[0], available: true, featured: false })
  const save = async () => {
    if (!editing) return
    if (!editing.name || !editing.category) { push('Name & category required', 'error'); return }
    setBusy(true)
    const res = await fetch('/api/food', { method: editing.id ? 'PUT' : 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(editing) })
    setBusy(false)
    if (res.ok) { push('Saved', 'success'); setEditing(null); load() } else push('Save failed', 'error')
  }
  const remove = async (id: string) => {
    if (!confirm('Delete this dish?')) return
    const res = await fetch('/api/food', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id }) })
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div><div className="text-xs uppercase tracking-[0.4em] text-clay">Manage</div><h1 className="font-display text-4xl text-forest mt-2">Food Menu</h1></div>
        <button onClick={openNew} className="px-5 py-3 rounded-full bg-forest text-cream font-semibold flex items-center gap-2"><Plus className="w-4 h-4" /> Add Item</button>
      </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
          {items.map(f => (
            <div key={f.id} className="rounded-2xl overflow-hidden bg-white border border-forest/5 flex flex-col">
              <div className="h-40 bg-cream"><img src={f.image_url} alt={f.name} className="w-full h-full object-cover" /></div>
              <div className="p-4 flex-1 flex flex-col">
                <div className="text-[10px] uppercase tracking-widest text-clay">{f.category}</div>
                <div className="flex items-baseline justify-between mt-1"><div className="font-display text-lg text-forest">{f.name}</div><div className="font-semibold text-forest">{KSH(f.price)}</div></div>
                <p className="text-xs text-earth mt-2 flex-1 line-clamp-2">{f.description}</p>
                <div className="flex gap-2 mt-3">
                  <span className={`px-2 py-1 rounded-full text-[10px] font-semibold ${f.available ? 'bg-forest/10 text-forest' : 'bg-clay/10 text-clay'}`}>{f.available ? 'Available' : 'Unavailable'}</span>
                  {f.featured && <span className="px-2 py-1 rounded-full bg-sand text-forest-deep text-[10px] font-semibold">Featured</span>}
                </div>
                <div className="flex gap-2 mt-3">
                  <button onClick={() => setEditing(f)} className="flex-1 px-3 py-2 rounded-full bg-forest/10 text-forest text-xs font-semibold flex items-center justify-center gap-1"><Pencil className="w-3 h-3" /> Edit</button>
                  <button onClick={() => remove(f.id)} className="px-3 py-2 rounded-full bg-clay/10 text-clay text-xs font-semibold"><Trash2 className="w-3 h-3" /></button>
                </div>
              </div>
            </div>
          ))}
      ...
      {editing && (
        <div className="fixed inset-0 z-[80] grid place-items-center p-4">
          ...
          <div className="relative w-full max-w-xl max-h-[92vh] overflow-y-auto bg-cream rounded-3xl shadow-2xl">
            <div className="bg-forest text-cream px-8 py-6 flex items-center justify-between"><div className="font-display text-2xl">{editing.id ? 'Edit' : 'New'} Dish</div><button onClick={() => setEditing(null)} className="w-10 h-10 rounded-full bg-cream/10 grid place-items-center"><X className="w-5 h-5" /></button></div>
            <div className="p-8 space-y-4">
              ...
              <ImageUploader label="Dish image" value={editing.image_url} onChange={(url) => setEditing({ ...editing, image_url: url })} folder="food" />
